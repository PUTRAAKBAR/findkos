...
import { Search, Calendar, Clock, CheckCircle2, XCircle, FileText } from "lucide-react";
import { updateBookingStatus } from "@/app/actions/booking";
import { useRouter } from "next/navigation";

export default function PesananClient({ initialBookings }: { initialBookings: any[] }) {
  ...
  const handleAction = async (bookingId: string, status: 'disetujui' | 'ditolak') => {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground">Kelola permintaan sewa kos dari calon penyewa.</p>
      </div>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex md:flex-col gap-2 justify-end md:justify-center border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-6 min-w-[140px]">
                  {booking.status === 'menunggu' ? (
                    <>
                      <Button 
                        onClick={() => handleAction(booking.id, 'disetujui')} 
                      ...
                      <Button 
                        onClick={() => handleAction(booking.id, 'ditolak')} 
                        variant="outline" 
